...
func TestPostgresStore_RoundTrip(t *testing.T) {
	...
	in, _ := json.Marshal(Input{HeightCm: 40, WidthCm: 50, MoldingRate: 120, GlassRate: 800})
	saved, err := store.Save(Record{Input: in, Headline: 586, Label: "small"})
	if err != nil {
		t.Fatalf("save: %v", err)
	}
	if saved.ID == 0 {
	...
	if len(items) == 0 || items[0].ID != saved.ID || items[0].Label != "small" {
